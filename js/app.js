...
function retrieveSettings() {
    ...
    if (setting === 'dark') {
        ...
        mode.insertAdjacentHTML('afterbegin', moon);
        mode.style.color = colorSecondary;
        body.style.backgroundColor = colorPrimary;
        topNav.forEach(nav => nav.style.color = colorSecondary);
        navBackground.style.backgroundColor = colorPrimary;
        brand.style.color = colorSecondary;
        cover.style.backgroundColor = colorPrimary;
        h1.style.color = '#FFFFFF';
        coverH2.forEach(p => p.style.color = colorSecondary);
        viewWork.style.color = colorSecondary;
        portWrap.style.color = colorSecondary;
        portWrap.style.backgroundColor = colorPrimary;
        cards.forEach(card => card.style.backgroundColor = colorTeritary);
        aboutH2.style.color = colorSecondary;
        aboutP.forEach(p => p.style.color = colorSecondary);
        footNav.forEach(nav => nav.style.color = colorSecondary);
        footWrap.style.borderTop = `1px solid ${colorSecondary}`;
        linkedin.style.fill = colorSecondary;
        github.style.fill = colorSecondary;
        copyright.style.color = colorSecondary;
    }
}
...
gLink.addEventListener('mouseout', () => {
    if (localStorage.getItem('mode') === 'dark'){
        ...
        github.style.fill = colorPrimary; 
    }   
});
...
lLink.addEventListener('mouseout', () => {
    if (localStorage.getItem('mode') === 'dark'){
        ...
        linkedin.style.fill = colorPrimary; 
    }   
});
...
mode.addEventListener('click', (e) => {
    ...
    if (localStorage.getItem('mode') === 'dark') {
        ...
        let svg = document.querySelector('#modeSVG');
        mode.removeChild(svg);
        mode.insertAdjacentHTML('afterbegin', sun);
        mode.style.color = colorPrimary;
        body.style.backgroundColor = colorSecondary;
        topNav.forEach(nav => nav.style.color = colorPrimary);
        navBackground.style.backgroundColor = colorSecondary;
        brand.style.color = colorPrimary;
        cover.style.backgroundColor = colorSecondary;
        h1.style.color = '#000000';
        coverH2.forEach(p => p.style.color = colorTeritary);
        viewWork.style.color = colorTeritary;
        portWrap.style.color = colorPrimary;
        portWrap.style.backgroundColor = colorSecondary;
        cards.forEach(card => {
            card.style.color = colorSecondary;
            card.style.backgroundColor = colorPrimary;
        });
        aboutH2.style.color = colorPrimary;
        aboutP.forEach(p => p.style.color = colorPrimary);
        footNav.forEach(nav => nav.style.color = colorPrimary);
        footWrap.style.borderTop = `1px solid ${colorPrimary}`;
        linkedin.style.fill = colorPrimary;
        github.style.fill = colorPrimary;
        copyright.style.color = colorPrimary;
    }
